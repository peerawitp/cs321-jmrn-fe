...
const OrderDetail: React.FC = () => {
    ...
    const [order, setOrder] = useState(orderHistory.find((o) => o.orderId === orderId));
    ...
    return (
        <div className="min-h-screen bg-gray-100 p-8">
            <div className="max-w-4xl mx-auto bg-white p-6 rounded shadow-lg">
                <h1 className="text-3xl font-bold mb-6 text-center">Order #{order.orderId}</h1>
                <p className="text-lg text-center mb-6">Here are the details of your order.</p>

                {/* รายละเอียดคำสั่งซื้อ */}
                <div className="bg-gray-50 p-6 rounded mb-6 shadow-md">
                    <h2 className="text-xl font-bold mb-4">Order Details</h2>
                    <p className="text-gray-700">Order Date: {order.date}</p>
                    <p className="text-gray-700 mb-4">Total: ${order.total.toFixed(2)}</p>
                    <p className="text-gray-700 mb-4">Status: <span className="font-semibold">{order.status}</span></p>
                            ...
                            {order.items.map((item, index) => {
                                ...
                                return (
                                    <li key={index} className="flex items-center justify-between">
                                        ...
                                        <p className="text-lg font-semibold">${(productSize?.price * item.quantity).toFixed(2)}</p>
                                    </li>
                                );
                            })}
                        </ul>
