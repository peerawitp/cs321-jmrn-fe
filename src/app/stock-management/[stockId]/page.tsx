...
const ProductDetail = ({ params }: { params: { stockId: number } }) => {
  ...
  const { register, handleSubmit, reset, getValues } = useForm<AddProductSize>({
    defaultValues: {
      productId: 0 || Number(params.stockId),
      name: "",
      overallDiameter: 0,
      overallWidth: 0,
      measurementRim: "",
  ...
  const updateHandler = () => {
    ...
    if (!selectedID) {
      alert("Please select a product size to update");
      return;
    }
    ...
    updateProductSizeForm.overallDiameter = Number(
      updateProductSizeForm.overallDiameter,
    );
    updateProductSizeForm.overallWidth = Number(
      updateProductSizeForm.overallWidth,
    );
    updateProductSizeForm.price = Number(updateProductSizeForm.price);
    updateProductSizeForm.quantity = Number(updateProductSizeForm.quantity);
  ...
  const filteredProductSizes = currentProduct
    ? currentProduct.productSizes.filter((productSize) =>
        productSize.name.toLowerCase().includes(searchTerm.toLowerCase()),
      )
    : [];
